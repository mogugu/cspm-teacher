    ...
    <div class="work-temp-top clear">
      ...
      <div class="TH-card test-question-select fl">
        ...
        <div class="test-question-select-item test-question-select-item-search inline-block relative">
          <p class="TH-f-c-fade fs-14 tqsi-title">关键字搜索</p>
          <input @keyup.enter="handleDrop('keyword')" type="text" class="otl-none ptsi-input" placeholder="请输入关键字" v-model="searchOption.keyword">
    ...
    <div class="work-shop">
      <div class="work-shop-info clear fs-13 TH-f-c-default">
        <p class="work-shop-info-left fl">
          ...
          <span @click="handleDrop('orderType',1)" :class="{'TH-f-c-link': searchOption.orderType == 1}" class="work-shop-info-item cp">预览 <i>↓</i> </span>
          <span @click="handleDrop('orderType',2)" :class="{'TH-f-c-link': searchOption.orderType == 2}" class="work-shop-info-item cp">下载 <i>↓</i> </span>
        </p>
        ...
        <div class="cmwl-item TH-card inline-block relative" v-for="(item, $index) in wList"  @click="worksItemOpen(item)">
          ...
          <div class="cmwl-item-right fl TH-f-c-default">
            ...
            <p class="cmwl-item-right-desc ws-no fs-13">
              <span>大小：{{fileSize(item.workSize)}}</span>
              <span>预览：{{item.viewTimes}}</span>
              <span>下载：{{item.worksDownloads}}</span>
              <!-- <span>评分：{{item.workScore}}</span> -->
            </p>
            ...
            <!-- <p class="cmwl-item-uncollect absolute fs-14 TH-f-c-default cp">取消收藏</p> -->
...
export default {
  name: 'PersonalCollectWorkPlatform',
  components: {
  ...
  methods: {
    ...
    handleDrop(type, item) {
      ...
      this.searchOption.page = 1
      if (type == 'file') {
        this.searchOption.fileType = item?this.$store.dropdown.fileType[+item].text:''
      }
    ...
    searchByOption() {
      ...
      this.$http.post(ADDRESS.WORK_FAV_LIST_PLATFORM_ADDR, this.searchOption)
        .then((res) => {
          if (res.body.code == 1) {
    ...
    handleFav(item, $index, type) {
      callLoading.activeLoading()
      this.$http.post(ADDRESS.SOURCEREPO_WORK_PLATFORM_FAVORITE_ADDR, {
