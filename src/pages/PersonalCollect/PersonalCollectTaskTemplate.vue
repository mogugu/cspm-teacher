...
<template>
  <div class="container">
    ...
    <div class="work-temp-list clear">
      ...
      <div @click="$router.push({path:'/sourceRepo/workTemplate/detail',query:{id:item.tempId, isByMe:searchOption.condition=='buy'}})" class="wtl-item TH-card inline-block relative fl cp" v-for="(item, $index) in workList">
        ...
        <p v-if="searchOption.condition == 'favor'" @click.stop="handleFav(item, $index)" :class="{'wtl-item-uncollect': item.favoriteStatus, 'wtl-item-collect': !item.favoriteStatus}" class="cp fs-14 TH-f-c-default absolute">
          <span v-if="item.favoriteStatus">取消收藏</span>
          <span v-if="!item.favoriteStatus">收藏</span>
        </p>
      </div>
      <div class="wtl-pages" v-if="workPage.total > 10">
        <el-pagination
...
export default {
  name: 'PersonalCollectTaskTemplate',
  components: {
  ...
  created () {
    this.searchOption.condition = this.$route.query.condition || ''
    this.searchByOption()
  },
  watch: {
    '$route'() {
