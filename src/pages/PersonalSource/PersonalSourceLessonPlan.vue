...
<template>
  <div class="container">
    <div class="work-temp-top clear">
      <div class="work-temp-top-left fl">
        ...
        <div class="TH-card courseware-select TH-f-c-default fs-16 cp fl">
          ...
          <div class="courseware-select-item inline-block">
            ...
            <div class="wtt-search-main relative">
                <input @keyup.enter="handleDrop('keyword')" class="otl-none wtt-search-main-input" type="text" placeholder="请输入关键字..." v-model="searchOption.fileTitle">
                <img @click="handleDrop('keyword')" class="wtt-search-main-icon absolute" src="~assets/imgs/classTask/search.png" alt="search">
            </div>
          </div>
        </div>
      </div>
      <div class="work-temp-top-right fr TH-button-primary cp" @click="$router.push('/personalSource/lessonPlan/upload')">
        <img class="paper-top-button-icon" src="~assets/imgs/classTask/publish-task.png" alt="air">
        <span class="fs-20">上传教案</span>
      </div>
    </div>
    ...
    <div class="work-temp-list">
      <div class="TF-temp-Arr-space" v-if="coursewareList==''">
        <img src="~assets/imgs/codeZero/coll/no-lesson-plan.png" alt="">
        <p class="fs-18 TH-f-c-default">没有你需要的教案呢！</p>
      </div>
      <div v-if="coursewareList!=''" class="cmwl-item TH-card inline-block fl"  v-for="item in coursewareList" @click="previewMaterial(item)">
        ...
        <div class="cmwl-item-right fl TH-f-c-default">
          ...
          <p class="cmwl-item-right-author fs-14 TH-f-c-link">{{item.userName}}</span></p>
        </div>
      </div>
      <div class="swp-page" v-if="materialsPage.total > 9 &&coursewareList!=''">
        <el-pagination
...
export default {
  ...
  methods: {
    ...
    searchByOption() {
      ...
      this.$http.post(ADDRESS.GET_MATER_ALL_LISTS, this.searchOption)
        .then((res) => {
    ...
    previewMaterial(item){
      window.open(ADDRESS.SOURCEREPO_PREVIEW_MATERIAL+'?materialId='+item.materialId)
    }
  }
}
</script>
