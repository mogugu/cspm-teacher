...
<template>
  <div class="container">
    <div class="paper-top relative">
      ...
      <div @click="$router.push('/personalSource/testQuestion/prevGenerate')" class="TH-button-primary paper-top-button absolute cp">
        <img class="paper-top-button-icon" src="~assets/imgs/classTask/publish-task.png" alt="air">
        <span class="fs-20">立即组卷</span>
      </div>
    </div>
    <div class="paper-list-container">
      ...
      <div class="paper-list-content clear">
        ...
        <div v-for="item in paperList" @click="$router.push({path:'/personalSourceDetail/paper',query:{'id':item.paperId}})" class="cp fl paper-list-content-item TH-card inline-block relative">
          <div class="TH-f-c-default cp paper-title">
            <span class="fs-16 fwb tof inline-block" :title="item.paperTitle">{{item.paperTitle}}</span>
          </div>
          <div class="plci-info">
            ...
            <!-- <p @click.stop="handleFav(item,!item.favorite)" class="fr plci-info-fav ta-c cp" :class="{'plci-info-fav-active': item.favorite}">
              <span :class="{'plci-info-fav-icon-active': item.favorite}" class="plci-info-fav-icon inline-block"></span>
              <span class="plci-info-fav-text fs-14 TH-f-c-default"><i v-if="item.favorite">已收藏</i><i v-else="!item.favorite">收藏</i></span>
            </p> -->
          </div>
          <p class="TH-f-c-fade fs-14">{{item.updateTime.substring(0,10)}} 更新</p>
        </div>
      </div>
    </div>
