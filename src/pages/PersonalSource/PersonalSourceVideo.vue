  ...
  <div class="container">
    <div class="work-temp-top clear relative">
      <div @click="activityOption.showChoiceFlag = !activityOption.showChoiceFlag" class="TH-card wtt-choice TH-f-c-default ta-c fs-20 cp fl">
        ...
        <span>已涵盖 <i class="TH-f-c-link fs-26 fwb">{{$store.coverTree.video.count}}</i> 个活动 </span>
      </div>
      ...
      <div @click="$router.push('/personalSource/video/uploadAli')" class="TH-button-primary paper-top-button absolute cp">
        <img class="paper-top-button-icon" src="~assets/imgs/classTask/publish-task.png" alt="air">
        <span class="fs-20">上传视频</span>
      </div>
    </div>
    ...
    <div class="work-temp-list">
      <div class="video-list">
        ...
        <div @click="$router.push({path: '/personalSourceDetail/video', query: {id:item.videoId}})" @mouseover="item.mouseUpDown=true" @mouseleave="item.mouseUpDown=false" class="cp video-list-item TH-card inline-block" v-for="item in videoList">
          <div class="video-list-item-top relative">
            <img :src="ImgFun(item.imgUrl)" onerror="this.src='./static/imgs/source/pic2.png'" alt="rp">
...
export default {
  ...
  computed: {
    selectOption() {
      return deepClone(this.$store.coverTree.video.tree)
    }
  },
  created () {
  ...
  methods: {
    ...
    getSelectOption(cb) {
      if (this.$store.coverTree.video.tree.length > 0) {
        // this.selectOption = deepClone(this.$store.coverTree.video)
        cb()
        return
      }
      this.$http.get(ADDRESS.RESOURCE_COVER_VIDEO_NEW_ADDR)
        .then((res) => {
          if(res.body.code == 1) {
            this.$store.coverTree.video.count = res.body.data.size
            this.$store.coverTree.video.tree = res.body.data.tree[0].children.concat(res.body.data.tree[1].children)
            // this.selectOption = deepClone(this.$store.coverTree.video)
