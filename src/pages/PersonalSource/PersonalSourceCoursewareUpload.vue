  ...
  methods: {
    ...
    uploadVideoType(response,file,fileList) {
      if(response.code==1){
        ...
        console.log(this.materials.url);
    ...
    addMaterialsInfo(){
      if(this.activityOption.choosenActivity[0].label=='') {
        ...
        .then((res) => {
          ...
          if(res.body.code==1){
           callToast('课件上传成功！','success')
            this.$router.push('/personalSource/my/courseware')
          }else{
            callToast('课件上传失败！','error')
          }
        }, (err) => {
          callLoading.deactiveLoading()
          callToast('课件上传失败！','error')
    ...
    uploadMaterials(){
      if(this.materials.url==''){
        callToast('您还没有上传课件哦','warning')
      }else if(this.materials.name==''){
        callToast('请填写课件名称','warning')
      }else{
    ...
    beforeAvatarUpload(file){
      ...
      if (callMimeTypeCheck(file, type)) {
        ...
        if (!isLt500) {
          callToast('上传课件大小不能超过 100MB！','warning')
        }
        return isLt500
      }
      else {
