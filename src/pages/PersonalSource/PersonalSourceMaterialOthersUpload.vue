  ...
  methods: {
    ...
    addMaterialsInfo(){
       if(this.activityOption.choosenActivity[0].label=='') {
        ...
        .then((res) => {
          ...
          if(res.body.code==1){
           callToast('素材上传成功！','success')
           this.$router.push('/personalSource/my/materialOthers')
          }else{
            callToast('素材上传失败！','error')
          }
        }, (err) => {
          callLoading.deactiveLoading()
          callToast('素材上传失败！','error')
    ...
    uploadMaterials(){
      if(this.materials.url==''){
        callToast('您还没有上传素材哦','warning')
      }else if(this.materials.name==''){
        callToast('请填写素材名称','warning')
      }else{
    ...
    beforeAvatarUpload(file){
      ...
      if (callMimeTypeCheck(file, type)) {
        ...
        if (!isLt500) {
          callToast('上传素材大小不能超过 100MB！','warning')
        }
        return isLt500
      }
      else {
