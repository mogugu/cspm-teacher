    ...
    <div class="upload-img-default-first-step">
      <div class="uplode-inner-video ">
        <div class="upload-other-message clear">
          <div class="upload-video-message-list fl">
              <ul class="fs-16 form-width">
                <li>
                  ...
                  <input type="text" maxlength="20" v-model="contentInfo.videoName">
                </li>
              </ul>
          ...
          <div class="upload-video-message-list fl">
              <div class="upload-img-url relative">
                 <p class="disableColor fs-16">封面：</p>
                 <img v-if="contentInfo.imgUrl" :src="contentInfo.imgUrl" onerror="this.src='./static/imgs/source/pic1.png'" class="upload-img-url-detail">
                ...
                <el-upload
                  ...
                  <span class="avatar-uploader-text TH-f-c-link fs-16">更换封面</span>
                </el-upload>
              </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getContentInfo() {
      ...
      .then((res) => {
        if (res.body.code == 1) {
          ...
          this.contentInfo = deepClone(res.body.data)
          this.activityOption.choosenActivity.splice(0, 1, {
            label: res.body.data.categoryName,
            id: res.body.data.categoryId
          });
        }
        else {
    ...
    handleAvatarSuccess(res, file) {
      // this.contentInfo.imageUrl = URL.createObjectURL(file.raw);
      this.contentInfo.imgUrl = HEAD + res.data;
    },
    comfirmModify() {
      ...
      this.$confirm('此操作将修改此视频, 是否继续?', '提示', {
      ...
      .then(() => {
        callLoading.activeLoading()
        this.$http.post(ADDRESS.MY_SOURCE_EDIT_VIDEO, {
            videoId: this.videoId,
            videoName: this.contentInfo.videoName,
            imgUrl: this.contentInfo.imgUrl,
            disciplineCode: 12,
            gradeNo: 7,
            categoryId: this.activityOption.choosenActivity[0].id,
            categoryName: this.activityOption.choosenActivity[0].label
        })
        .then((res) => {
          if (res.body.code == 1) {
    ...
    resetModify() {
      console.log(this.contentInfoDeep,this.contentInfo)
