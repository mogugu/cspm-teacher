...
<template>
  <div class="container">
    <div class="work-temp-top  clear">
      <div class="work-temp-top-left fl inline-block">
        ...
        <div class="TH-card courseware-select TH-f-c-default fs-16 cp fl">
          ...
          <div class="courseware-select-item inline-block">
            ...
            <div class="wtt-search-main relative">
              <input @keyup.enter="handleDrop('keyword')" class="otl-none wtt-search-main-input" type="text" placeholder="请输入关键字..." v-model="searchOption.activityName">
              <img @click="handleDrop('keyword')" class="wtt-search-main-icon absolute" src="~assets/imgs/classTask/search.png" alt="search">
            </div>
          </div>
        </div>
      </div>
      <div class="work-temp-top-right fr TH-button-primary cp" @click="$router.push('/personalSource/courseware/upload')">
        <img class="paper-top-button-icon" src="~assets/imgs/classTask/publish-task.png" alt="air">
        <span class="fs-20">上传课件</span>
      </div>
    </div>
    ...
    <div class="work-temp-list">
      ...
      <div v-if="coursewareList!=''" class="cmwl-item TH-card inline-block relative"  v-for="item in coursewareList" @click="previewMaterial(item)">
